...
variable "private_key" {
  type = map(string)
  default = {
    "name" = "mosipuser"  // Name as given while creating keys on AWS console 
    "local_path" = "~/.ssh/mosipuser.pem" // Location on the machine from where you are running terraform
  } 
}
