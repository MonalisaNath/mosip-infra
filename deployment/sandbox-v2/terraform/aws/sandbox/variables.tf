...
variable "private_key" {
  type = map(string)
  default = {
    "name" = "mosip-newaws"  // Name as given while creating keys on AWS console 
    "local_path" = "~/.ssh/mosip-newaws.pem" // Location on the machine from where you are running terraform
  } 
}
